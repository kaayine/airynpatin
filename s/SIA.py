...
def signup():
    if request.method == "POST":
        ...
        try:
            ...
            if existing.data:
                return "<h3 style='color:white;text-align:center;'>⚠ Email sudah terdaftar. Silakan <a href='/signin' style='color:#ffcc00;'>Login</a>.</h3>"
            ...
            if email_status:
                ...
                return f"""
                <h3 style='color:white;text-align:center;'>❌ Gagal mengirim kode OTP ke {email}.</h3>
                <p style='color:#ccc;text-align:center;'>Pastikan email benar, atau cek konfigurasi server (App Password).</p>
                <div style='text-align:center;'><a href='/signup' style='color:#ffcc00;'>Coba Lagi</a></div>
                """
        except Exception as e:
            print(f"Error signup: {e}")
            return f"<h3 style='color:white;text-align:center;'>❌ Terjadi kesalahan sistem: {e}</h3>"

    return render_template_string(signup_content)

@app.route("/verify", methods=["GET", "POST"])
def verify():
    # Pastikan ada data user sementara di session
        ...
        if user_otp == stored_otp:
            ...
            try:
                ...
                data = {
                    "email": email,
                    "password": password
                    # Jika tabel users Anda punya kolom created_at, biarkan default/auto
                }
                ...
                return f"<h3 style='color:white;text-align:center;'>❌ Gagal menyimpan ke database.<br><small>{e}</small></h3>"
        else:
            return "<h3 style='color:white;text-align:center;'>❌ Kode OTP Salah. Silakan coba lagi.</h3>"
